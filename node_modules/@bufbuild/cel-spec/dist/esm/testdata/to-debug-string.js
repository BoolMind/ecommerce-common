...
const UNPRINTABLE_EXP = /[^\p{L}\p{N}\p{S}\p{P}\p{Cs} ]/v;
// @ts-expect-error - The regex flag `v` is only available in ES2024 or later
const UNPRINTABLE_EXP_GLOBAL = /[^\p{L}\p{N}\p{S}\p{P}\p{Cs} ]/gv;
const SPECIAL_ESCAPES = new Map([
    [0x07, "\\a"],
    [0x08, "\\b"],
    [0x09, "\\t"],
    [0x0a, "\\n"],
 ...
 */
export function toDebugString(expr, adorner = EmptyAdorner.singleton) {
...
class EmptyAdorner {
    static singleton = new EmptyAdorner();
    constructor() { }
    GetMetadata(_context) {
        return "";
    }
}
export class KindAdorner {
    static singleton = new KindAdorner();
    constructor() { }
    GetMetadata(context) {
        let valueType;
        if (isExpr(context)) {
...
function isSegmentPrintable(s, reserved = ["\\", '"']) {
    if (reserved.includes(s))
        return false;
    if (UNPRINTABLE_EXP.test(s.normalize()))
        return false;
    try {
...
function escapeByte(value) {
    return (SPECIAL_ESCAPES.get(value) ?? "\\x" + value.toString(16).padStart(2, "0"));
}
