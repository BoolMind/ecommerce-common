...
import { ConcreteAttributeFactory, ErrorAttr, } from "./access.js";
import { VarActivation } from "./activation.js";
import * as opc from "./gen/dev/cel/expr/operator_const.js";
import { Namespace } from "./namespace.js";
import { celError, celErrorMerge, isCelError, } from "./error.js";
...
export class Planner {
    ...
    namespace;
    factory;
    constructor(functions, registry, namespace = Namespace.ROOT) {
    ...
    planCall(id, call) {
        ...
        if (call.target !== undefined) {
            ...
            if (qualName !== undefined) {
                ...
                for (const candidate of this.namespace.resolveCandidateNames(funcName)) {
                    ...
                    if (func !== undefined) {
                        return new EvalCall(id, candidate, "", undefined, func, call.args.map((arg) => this.plan(arg)));
                    }
                }
            }
        }
        ...
        switch (call.function) {
            ...
            default:
                return new EvalCall(id, call.function, "", call.target ? args[0] : undefined, this.functions.find(call.function), call.target ? args.slice(1) : args);
        }
    }
...
class EvalCall {
    ...
    func;
    args;
    constructor(id, name, overload, target, func, args) {
        this.id = id;
        this.name = name;
    ...
    eval(ctx) {
        ...
        return celError(`found no matching overload for '${this.name}' applied to '${target !== undefined ? `${celType(target)}.` : ""}(${vals
            .map((x) => celType(x))
            .map((x) => x.name)
            .join(", ")})'`, this.id);
    }
...
class EvalObj {
    ...
    eval(ctx) {
        ...
        try {
            return celObject(this.typeName, obj);
        }
        catch (ex) {
...
class EvalFold {
    ...
    eval(ctx) {
        ...
        if (isCelMap(iterRange)) {
            ...
            return celError(`type mismatch: iterable vs ${celType(iterRange)}`, this.id);
        }
        // Fold the items.
    ...
    eval(ctx) {
        ...
        for (const arg of this.args) {
            ...
            if (isCelError(val)) {
                ...
                errors.push(celError(`type mismatch: expected bool, got ${celType(val)}`, this.id));
            }
        }
        if (errors.length > 0) {
