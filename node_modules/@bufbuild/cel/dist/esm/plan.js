...
import { celError, isCelError } from "./error.js";
...
export function plan(env, expr) {
    ...
    const withContext = withEvalContext({ registry: env.registry }, {
        id: plan.id,
        eval(ctx) {
            try {
    ...
    return (ctx) => {
        return withContext.eval(ctx !== undefined ? new ObjectActivation(ctx) : EMPTY_ACTIVATION);
    };
}
